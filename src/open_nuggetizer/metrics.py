...
def calculate_nugget_scores(qid: str, nuggets: List[Dict]) -> NuggetMetrics:
    ...
    strict_weighted_supported = sum(1 for n in vital_nuggets if n['assignment'] == 'support') + 0.5*sum(1 for n in okay_nuggets if n['assignment'] == 'support')
    ...
    weighted_supported = strict_weighted_supported + sum(0.5 for n in vital_nuggets if n['assignment'] == 'partial_support') + 0.5*sum(0.5 for n in okay_nuggets if n['assignment'] == 'partial_support')
    ...
    strict_weighted_score = strict_weighted_supported / (len(vital_nuggets) + 0.5*len(okay_nuggets)) if (len(vital_nuggets) + 0.5*len(okay_nuggets)) else 0.0
    ...
    weighted_score = weighted_supported / (len(vital_nuggets) + 0.5*len(okay_nuggets)) if (len(vital_nuggets) + 0.5*len(okay_nuggets)) else 0.0
